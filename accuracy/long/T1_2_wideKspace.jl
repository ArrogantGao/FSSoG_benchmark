...
K_max_array = [5,10,15,20,25,30]
uspara_array = [USeriesPara(i...) for i in 1:6]
uspara = uspara_array[6]
M_mid_array = [1:1:25...]
M_total = length(uspara.sw)
E_kspace=[]
E_direct_total = long_energy_us_k(qs, poses, 100, L, uspara, 1, M_total) + long_energy_us_0(qs, poses, L, uspara, 1, M_total)
...
for j in 1:length(M_mid_array)
    ...
    global E_direct_true -= long_energy_us_k(qs, poses, 100, L, uspara, M_mid, M_mid) + long_energy_us_0(qs, poses, L, uspara, M_mid, M_mid)
    @show E_direct_true
    ...
    for i in 1:length(K_max_array)
        ...
        CSV.write("data/Acc_T1_2_wideKspace.csv", df, append = true)
    end
end
