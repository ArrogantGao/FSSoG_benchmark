...
CSV.write("data/Acc_T5_thin_w.csv", DataFrame(preset = Int[], w = Int[], gamma = Int[], energy_exact = Float64[], energy_k = Float64[], abs_error = Float64[], relative_error = Float64[]))
...
for gamma in gammas
    ...
    for w in ws
        ...
        df = DataFrame(w = w[1], gamma = gamma, E_exact = E_direct_k, E_long = E_thin_k, error_rel = error_rel)
        CSV.write("data/Acc_T5_thin_w.csv", df, append = true)
