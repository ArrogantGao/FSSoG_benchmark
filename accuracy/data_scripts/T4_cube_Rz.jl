...
CSV.write("data/Acc_T4_cube_Rz.csv", DataFrame(preset = Int[], N_z = Int[], M_mid = Int[], energy_exact = Float64[], energy_k = Float64[], abs_error = Float64[], relative_error = Float64[]))
...
for i in 1:length(M_mid_array)
    ...
    for N_z in N_z_array
        ...
        df = DataFrame(R_z = N_z, M_mid = M_mid, E_exact = E_direct_k, E_FFCT = E_FFCT_loop_k, abs_error = abs_error, relative_error = relative_error)
