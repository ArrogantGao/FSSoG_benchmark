...
CSV.write("data/Acc_T3_1_WinFunc_PKB.csv", DataFrame(preset = Int[], w = Int[], energy_exact = Float64[], energy_k = Float64[], abs_error = Float64[], relative_error = Float64[]))
...
for i in 1:length(w_array)
    ...
    alpha = π.*0.91 .* w 
        ...
        df = DataFrame(Window_type = 1, energy_exact = E_direct_true, abs_error = error, relative_error = error_rel, w = w[1])
        #CSV.write("data/Acc_T1_2_WinFunc_Gaussian.csv", df, append = true)
        CSV.write("data/Acc_T3_1_WinFunc_PKB.csv", df, append = true)
end
