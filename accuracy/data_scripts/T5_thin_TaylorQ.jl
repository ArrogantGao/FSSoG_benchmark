...
CSV.write("data/Acc_T5_thin_TaylorQ.csv", DataFrame(preset = Int[], Taylor_Q = Int[], gamma = Int[], energy_exact = Float64[], energy_k = Float64[], abs_error = Float64[], relative_error = Float64[]))
...
for gamma in gammas
    ...
    for Taylor_Q in Taylor_Qs
        ...
        df = DataFrame(TaylorQ = Taylor_Q, gamma = gamma, E_exact = E_direct_k, E_long = E_thin_k, error_rel = error_rel)
        CSV.write("data/Acc_T5_thin_TaylorQ.csv", df, append = true)
        @show Taylor_Q, error_rel
    end
end