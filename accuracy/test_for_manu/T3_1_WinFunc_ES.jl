...
function mid_paras_gen_diffwindow(N_real::NTuple{3, Int}, w::NTuple{3, Int}, β::NTuple{3, T}, L::NTuple{3, T}, extra_pad_ratio::Int, cheb_order::Int, uspara::USeriesPara{T}, M_mid::Int, alpha::NTuple{3, T}) where{T}
    ...
    x1 = [i * gridinfo.h[1] for i in -Int(gridinfo.N_pad[1] / 2):Int(gridinfo.N_pad[1] / 2 - 1)]
    x2 = [i * gridinfo.h[2] for i in -Int(gridinfo.N_pad[2] / 2):Int(gridinfo.N_pad[2] / 2 - 1)]
    x3 = [i * gridinfo.h[3] for i in -Int(gridinfo.N_pad[3] / 2):Int(gridinfo.N_pad[3] / 2 - 1)]
...
for i in 1:length(w_array)
        ...
        df = DataFrame(Window_type = 3, energy_exact = E_direct_true, abs_error = error, relative_error = error_rel, w = w[1])
        #CSV.write("data/Acc_T1_2_WinFunc_Gaussian.csv", df, append = true)
        CSV.write("data_for_manu/Acc_T3_1_WinFunc_ES.csv", df, append = true)
end
