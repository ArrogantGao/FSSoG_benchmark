...
for i in 1:4
    M = M_mid[i]
    mask = df_xy.M_mid .== M
    scatter!(axl, N_xy, df_xy.error_rel[mask], markersize = 15, label = L"M_{\text{mid}} = %$M", marker = marker[i])
end

for i in 1:4
    M = M_mid[i]
    mask = df_z.M_mid .== M
    scatter!(axr, R_z, df_z.error_rel[mask], markersize = 15, label = L"M_{\text{mid}} = %$M", marker = marker[i])
end
